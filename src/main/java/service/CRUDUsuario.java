...
public class CRUDUsuario {
	...
	public boolean registrarUsuario(Usuario usuario) {
		...
		this.sql = "INSERT INTO usuario(documento,fecha_nac,nombre,"
				+ "apellido,correo,numeroContacto,nombreDeUsuario,contraseña) VALUE ('" + usuario.getDocumento() + "','"
				+ usuario.getFecha_nac() + "','" + usuario.getNombre() + "','" + usuario.getApellido() + "','"
				+ usuario.getEmail() + "','" + usuario.getNumeroContacto() + "','" + usuario.getNombreUsuario() + "','"
		...
		try {
			...
			while (resultado.next()) {
				usuarios.add(new Usuario(resultado.getString("nombre"), resultado.getString("apellido"),
						resultado.getString("email"), resultado.getString("documento"),
						resultado.getString("constraseña"), resultado.getString("nombreUsuario"),
	...
	public boolean iniciarSesion(Usuario user) {
		sql = "SELECT * FROM usuario WHERE (nombreDeUsuario=" + " '" + user.getNombreUsuario() + "' OR " + "correo = '"
				+ user.getEmail() + "')AND contraseña='" + user.getContraseña() + "'";
		try {
	...
	public boolean solicitarTarjetaDescuento(Usuario user) {
		...
		this.sql = "SELECT	count(*) as totalReservas from reserva " + "where id_usuario=" + user.getId()
				+ " AND (fecha_compra BETWEEN '" + intervaloInf + "' AND NOW() )";
		try {
			ResultSet res = this.conexion.getSentenciaSQL().executeQuery(sql);
			res.next();
			if (res.getInt("totalReservas") >= 6) {
				return actualizarDescuentos(user);
			}
			System.out.println("No cumple con los requisitos para solicitar la tarjeta de Descuento, tiene: "
					+ res.getInt("totalReservas") + "reservas en 3 meses");
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return false;
